...
class EditAndDeleteInDB : AppCompatActivity() {
    ...
    private val connection by lazy { CelebrityDatabase.getInstance(this).celebrityDao() }
    ...
    private fun updateCelebrity() {
        if (checkEntry()) {
            CoroutineScope(IO).launch {
                connection.updateCelebrity(Information(id, name, taboo1, taboo2, taboo3))
            }
    ...
    private fun deleteCelebrity() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("YES"){_,_ ->
                CoroutineScope(IO).launch {
                    connection.deleteCelebrity(Information(id, name, taboo1, taboo2, taboo3))
                }
                StyleableToast.makeText(
                    this,
    ...
    private fun updateList(){
        ...
        CoroutineScope(IO).launch {
            val data= async {
                connection.gettingAllCelebrities()
            }.await()
            if (data.isNotEmpty())
                withContext(Main){
                    list.addAll(data)
